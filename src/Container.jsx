...
export default class Container extends Component {
  constructor(props) {
    ...
    check = () => {
    if (this.props.valueClik === "ALL") {
      return this.state.SNEAKERS;
    } else if (this.props.valueClik === "search") {
      return this.state.SNEAKERS.filter((sneaker) =>
        sneaker.model.toLowerCase().includes(this.props.valueSearch.toLowerCase())
      );
    } else {
      return this.state.SNEAKERS.filter(
        (sneaker) => sneaker.brand === this.props.valueClik
      );
    }
  };
  render() {
    return (
      <div className="container">
        <div className="row">
          {this.check().map((item, index) => (
            <Producte data={item} key={index} />
          ))}
        </div>
      </div>
    );
